...
to setup ;; what happens when setup is clicked
  ...
  create-ents turtles-to-create ;; creates 1 ent from "ents"
  create-atwalkers turtles-to-create
    ask ents ;; changes things for every ent and not just one
  [ 
    set size turtle-size ;; changes the size of the ent
    set color blue ;; changes the colour of the ent
  ...
  set-default-shape atwalkers "atwalker"
...
This model shows the spawning of two turtles at random coordinates everytime the setup button is pressed.

## HOW IT WORKS

The model works by repeatedly pressing the setup button to observe the turtles new position.

## HOW TO USE IT

The setup button clears everything from the stage then spawns new turtles at random locations.
...
Have more than one turtle spawn at anytime.
Have random colours.
